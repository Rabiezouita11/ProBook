						  ...
						  @foreach($publications as $publication)
                                    ...
                                    @endif
												...
												<div class="friend-name">
													...
													<ins><a title="" href="time-line.html">{{Auth::user()->name}}</a> 
													<span><i class="icofont-globe"></i> published: {{ \Carbon\Carbon::parse($publication->created_at)->format('M, d Y') }}</span>
												</div>
@php

    // Assuming $publication->created_at is your timestamp
    $created_at = \Carbon\Carbon::parse($publication->created_at);
...
@endphp
    ...
    @if($publication->image)
			...
			<a data-toggle="modal" data-target="#img-comt" data-time="{{ $timeElapsed }}" data-src="{{ asset('images/' . $publication->image) }}">
...
<div class="modal fade" id="img-comt">
                        ...
                        <div class="modal-body">
                            <div class="row merged">
                                ...
                                <div class="col-lg-3">
                                    <div class="commentbar">
                                        <div class="user">
                                           	@if (Auth::user()->image)
							<figure><img src="{{ asset('users/' . Auth::user()->image) }}" height="50px" width="50px" alt=""></figure>
							@else
							<figure><img
									src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=104d93&color=fff"
									alt=""></figure>
							@endif
                                            <div class="user-information">
                                                <h4>
                                                    <a href="{{route('Profile_User')}}" title="">{{auth::user()->name}}</a>
                                                </h4>
                                        ...
                                        <div class="new-comment" style="display: block;">
											...
											@if(Auth::check() && $publication->user_id === Auth::id())
											...
											@if($publication->commentaires->isNotEmpty())
                                                <ul>
                                                    ...
                                                    <li>
                                                        ...
                                                        <div class="commenter">
                                                            <h5>
                                                                <a title="" href="#">{{ $comment->user->name }}</a>
                                                            </h5>
															@php
    ...
    // Calculate the difference between the created_at timestamp and the current time
    $timeElapsedd = $created_at->diffForHumans(null, true);
@endphp					
